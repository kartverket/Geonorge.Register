using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace Kartverket.Register.Helpers
{
    public static class GlobalVariables
    {
        public static string DokRegistryId
        {
            get { return "CD429E8B-2533-45D8-BCAA-86BC2CBDD0DD"; }
        }

        public static string InspireRegistryId
        {
            get { return "9A9BEF28-285B-477E-85F1-504F8227FF45"; }
        }

        public static string GeodatalovRegistryId
        {
            get { return "3D9114F6-FAAB-4521-BDF8-19EF6211E7D2"; }
        }

        public static string MareanoRegistryId
        {
            get { return "3D9114F6-FAAB-4521-BDF8-19EF6211E7D3"; }
        }

        public static string DokMunicipalRegistryId
        {
            get { return "E807439B-2BFC-4DA5-87C0-B40E7B0CDFB8"; }
        }

        public static string AlertRegistryId
        {
            get { return "0f428034-0b2d-4fb7-84ea-c547b872b418"; }
        }

        public static string ProductSheetsId
        {
            get { return "A42BC2B3-2314-4B7E-8007-71D9B10F2C04"; }
        }
        public static string ProductSpecificationId
        {
            get { return "8E726684-F216-4497-91BE-6AB2496A84D3"; }
        }
        
        public static string CartographyId
        {
            get { return "5EACB130-D61F-469D-8454-E96943491BA0"; }
        }
    }
}